...
Read-only, unauthenticated JSON API over the site's testimonial section. No API key,
...
|---|---|---|
| `GET /api/testimonials` | Convex DB (live) | Written testimonials — name, country, rating, review text, proof documents |
| `GET /api/testimonials/videos` | Static list | Video testimonials — YouTube links + thumbnails |

Both accept `OPTIONS` for CORS preflight. Only `GET` is exposed — there is no public
write path; testimonials are created through the admin panel.
...
## 2. `GET /api/testimonials/videos`

Video success stories (YouTube). Static content — it changes only on deploy, so it is
safe to cache aggressively on your side.
...
```json
{
  ...
  "meta": { "total": 17, "count": 1, "limit": 1, "offset": 0, "hasMore": true }
}
```

| Parameter | Type | Default | Description |
|---|---|---|---|
| `country` | csv | — | Exact country match, case-insensitive. Note some entries use `"Abroad"` as the country. |
| `q` | string | — | Substring search across name, profession, country, achievement, quote, details. |
| `limit` | int | `20` | Clamped 1–100. |
| `offset` | int | `0` | Rows to skip. |

`id` here is a small integer from the static list, **not** the same id space as written
testimonials. `duration` is a free-text label, not a runtime. `videoUrl` is always a
YouTube watch URL — the video id is the `v` query parameter if you need an embed.

```bash
curl "{BASE_URL}/api/testimonials/videos?country=Germany"
curl "{BASE_URL}/api/testimonials/videos?q=opportunity%20card&limit=5"
```
...
- **The written and video endpoints have different shapes.** Do not try to merge them without normalising first — videos have no `rating`, `review`, or `service`.
